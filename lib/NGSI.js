...
(function () {
    ...
    if ((typeof require == 'function') && typeof exports != null) {        
        NGSI = exports;
        NGSI.EventSource = require('./eventsource.js').EventSource
    } else {
        NGSI = {};
        NGSI.EventSource = window.EventSource
    }
    ...
    var makeJSONRequest = function makeJSONRequest(url, payload, parse_func, callbacks, parameters) {
        ...
        if (payload != null) {
            ...
            body = JSON.stringify(payload, function(k,v) {
              var t = typeof(v);
    ...
    var ngsi_build_entity_id_element_json = function ngsi_build_entity_id_element_json(entity) {
        entity.isPattern = entity.isPattern || false; // is it allowed to leave this out (makes life a lot easier!)
        return entity;
    };
    
    var ngsi_build_entity_id_elements_json = function ngsi_build_entity_id_elements_json(entities) {
        var result = [];
        var i;
        for (i = 0; i < entities.length; i += 1) {
            result.push(ngsi_build_entity_id_element_json(entities[i]));
        }
        return result;
    }
    ...
    var ngsi_build_register_context_request = function ngsi_build_register_context_request(e, attr, duration, providingApplication, regId) {
        ...
        doc = {
            contextRegistrations: [
                {
                    entities: ngsi_build_entity_id_elements_json(e),
                    attributes: attr,
    ...
    var ngsi_build_update_context_request = function ngsi_build_update_context_request(updateAction, update) {
       var  body = {
        ...
        for (i = 0; i < update.length; i += 1) {
            ...
            // Basic entity info
            var contextElement = ngsi_build_entity_id_element_json(update[i].entity);

            // attribute list
            if ('attributes' in update[i]) {
                contextElement.attributes = update[i].attributes;
            }
    ...
    var ngsi_build_discover_context_availability_request = function ngsi_build_discover_context_availability_request(e, attributes) {
        return {
            entities: ngsi_build_entity_id_elements_json(e),
    ...
    var ngsi_build_subscribe_update_context_availability_request = function ngsi_build_subscribe_update_context_availability_request(e, attr, duration, restriction, subscriptionId, onNotify) {
        ...
        if (restriction) {
            doc.restriction = restriction;
        }

        return doc;
    };

    var ngsi_build_unsubscribe_context_availability_request = function ngsi_build_unsubscribe_context_availability_request(subscriptionId) {
        return {
            subscriptionId: subscriptionId
        };
    };

    var ngsi_build_subscribe_update_context_request = function ngsi_build_subscribe_update_context_request(payload, onNotify) {
        payload.entities = ngsi_build_entity_id_elements_json(payload.entities)
        
        if (onNotify) payload.reference = onNotify

        return payload;
    };

    var ngsi_build_unsubscribe_context_request = function ngsi_build_unsubscribe_context_request(subscriptionId) {
        return {
            subscriptionId: subscriptionId
        };
    };
    ...
    var connect_to_eventsource = function connect_to_eventsource() {
        ...
        source = new NGSI.EventSource(this.source_url);
    ...
    // TODO: used params.reference in case notification not used
    NGSI.Connection.prototype.createSubscription = function createSubscription(params, options) {
        if (!Array.isArray(params.entities) || params.entities.length === 0) {
            throw new TypeError('entities parameter must be a non-empty array');
        }

        if (params.attributes != null && !Array.isArray(params.attributeNames)) {
            throw new TypeError('attributeNames parameter must be an array or null');
        }
        ...
        if (options.onNotify && this.ngsi_proxy != null) {
            ...
            this.ngsi_proxy.request_callback(onNotify, function (proxy_callback) {
                ...
                var payload = ngsi_build_subscribe_update_context_request(params, proxy_callback.url);
            ...
            var payload = ngsi_build_subscribe_update_context_request(params);
            makeJSONRequest.call(this, url, payload, parse_subscribe_context_response, options);
        }
    };

    NGSI.Connection.prototype.updateSubscription = function updateSubscription(payload, options) {
        if (!payload || payload.subscriptionId == null) {
            throw new TypeError('subId parameter cannot be null');
        }
