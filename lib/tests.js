...
describe("NGSI Library handles", function () {
    ...
    var connection, response, response_data, response_details, failure, ajaxMockup, eventsources;
    ...
    beforeEach(function () {
        ...
        if (this.env.currentSpec.description.indexOf('proxy') !== -1) {
            options.ngsi_proxy_url = 'http://ngsi-proxy.server.com';
        }
    ...
    it("connection errors while making query requests", function () {
    ...
    it("bad responses for query requests", function () {
    ...
    it("basic add context requests adding metadata", function () {
        ...
        runs(function () {
            ...
            ajaxMockup.addStaticURL("http://ngsi.server.com/ngsi10/updateContext", {
                    ...
                    responseText: request.responseText,
                    checkRequestContent: function (url, options) {
                        var doc, result, attribute, name, type, contextValue, metadata, value;
    ...
    it("basic subscribe context requests using the ngsi proxy", function () {
        ...
        runs(function () {
            ...
            ajaxMockup.addStaticURL("http://ngsi-proxy.server.com/eventsource", {
                status_code: 200,
                responseText: '',
                headers: {
                    'Location': 'http://ngsi-proxy.server.com/eventsource/1'
                }
            });

            ajaxMockup.addStaticURL("http://ngsi-proxy.server.com/callbacks", {
                status_code: 200,
                responseText: '{"callback_id": "1", "url": "http://ngsi-proxy.server.com/callback/1"}',
                checkRequestContent: function (url, options) {
            ...
            ajaxMockup.addStaticURL("http://ngsi.server.com/ngsi10/subscribeContext", {
                    ...
                    checkRequestContent: function (url, options) {
                        ...
                        result = doc.evaluate('reference', doc.documentElement, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
                        if (result.snapshotLength !== 1 || result.snapshotItem(0).textContent !== 'http://ngsi-proxy.server.com/callback/1') {
                            return false;
                        }
        ...
        runs(function () {
            ...
            expect(eventsources[0].url).toEqual('http://ngsi-proxy.server.com/eventsource/1');
