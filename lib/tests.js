...
/* globals ajaxMockFactory, mockedeventsources, EventSource, NGSI */
...
(function () {
    ...
    var _waitsForResponse = function _waitsForResponse(next, done, hop) {
        if (response) {
            ...
            setTimeout(_waitsForResponse.bind(null, next, done, hop + 1), 50);
        }
    };

    var waitsForResponse = function waitsForResponse(next, done) {
        setTimeout(_waitsForResponse.bind(null, next, done, 0), 0);
    };
    ...
    describe("NGSI Library handles", function () {
        ...
        describe('appendAttributes(changes[, options])', function () {
