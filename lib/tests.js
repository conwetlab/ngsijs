...
/*global ajaxMockFactory, EventSource, beforeEach, expect, it, NGSI, waitsFor*/

(function () {

    "use strict";

    var connection, response, response_data, response_error_data, response_details, failure, ajaxMockup, notification_data, ajaxMockFactory, EventSource, NGSI, XMLHttpRequest;

    /* Detect Node.js */
    /* istanbul ignore if */
    if ((typeof require == 'function')) {
        ajaxMockFactory = require('./helpers/AjaxMockFactory.js').ajaxMockFactory;
        EventSource = require('./helpers/EventSourceMock.js').EventSource;
        NGSI = require('./ngsi-node');
        XMLHttpRequest = require('./helpers/XMLHttpRequest.js');
    } 
    ...
    var bad_create_subscription_response_test = function bad_create_subscription_response_test(description, response_status, error_class) {
        it("create subscription error responses (" + description + ")", function (done) {
            ...
            connection.createSubscription({
                ...
                duration: 'PT24H',
                notifyConditions: [{type: 'ONCHANGE', condValues: ['position']}]
            },{
    ...
    describe("NGSI Library handles", function () {
        ...
        it("basic subscribe context availability requests", function (done) {
            ...
            waitsForResponse(function () {
                expect(failure).toEqual(false);
                expect(EventSource.mocks.length).toEqual(0);
                expect(response_data).toEqual({
                    'subscriptionId': 'sub1',
                    'duration': 'PT24H'
                });
        ...
        it("basic subscribe context requests", function (done) {
            ...
            connection.createSubscription({
                ...
                duration: 'PT24H',
                notifyConditions: [{type: 'ONCHANGE', condValues: ['position']}],
                reference: 'http://www.example.com/callback'
            }, {
                    onSuccess: function (data) {
                        response_data = data;
                    },
        ...
        // TODO: add proxy notification update
    ...
    describe("NGSI Library (using proxies) handles", function () {
        ...
        it("basic subscribe availability context requests using the ngsi proxy", function (done) {
            ...
            waitsForResponse(function () {
                //console.log(EventSource.prototype);
                expect(failure).toEqual(false);
                expect(EventSource.mocks.length).toEqual(1);
                expect(EventSource.mocks[0].url).toEqual('http://ngsiproxy.example.com/eventsource/1');
                expect(EventSource.mocks[0].events.init.length).toEqual(0);
                expect(EventSource.mocks[0].events.close.length).toEqual(1);
                expect(EventSource.mocks[0].events.notification.length).toEqual(1);
                expect(response_data).toEqual({
                    'subscriptionId': 'sub1',
                    'duration': 'PT24H'
                });
